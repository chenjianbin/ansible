...
class ZabbixInventory(object):
    ...
    def get_host(self, api, name):
        # 当zabbix版本小于6.2时, selectHostGroups替换为selectGroups
        api_query = {'output': 'extend', 'selectHostGroups': 'extend', "filter": {"host": [name]}}
    ...
    def get_list(self, api):
        ...
        for host in hosts_data:
            hostname = host['name']
            hostname = '.'.join(hostname.split('-')[0:4])
            hostvars = dict()
            data[self.defaultgroup]['hosts'].append(hostname)
            # 当zabbix版本小于6.2时, hostgroups替换为groups
            ...
            if 'interfaces' in host:
               # use first interface only
                if host['interfaces'][0]['useip'] == 0:
                    ...
                    hostvars['ansible_ssh_host'] = hostname
                else:
